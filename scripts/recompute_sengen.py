...
(common.is_sengen)で読み直し、docs/accuracy.jsonの該当日エントリのsen_n/sen_hit/
sen_pred_sumのみを差し替えてtotalを再集計する。Kやモデルは不要(予測JSON内の
picksと確定結果のorderだけで完結)。"""
import glob
import json
import os
import sys
...
from common import is_sengen, sengen_top5p
...
def recompute_day(pred: dict):
    ...
    for v in pred.get("venues", []):
        for r in v.get("races", []):
            ...
            if not is_sengen(top5p, v.get("code")):
                continue
            sen_n += 1
            sen_pred_sum += top5p
